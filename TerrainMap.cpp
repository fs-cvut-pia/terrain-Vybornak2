...
TerrainMap::TerrainMap(int n, int m, std::string filename) : TerrainMap(n,m) {
...
bool TerrainMap::validCoords(int x, int y) {
    if (x >= 0 && x < nx && y >= 0 && y < ny) return true;
    else return false;
}

bool TerrainMap::validCoords(Point const& p) {
    return validCoords(p.x, p.y);

}

int TerrainMap::alt(int x, int y) {
    if (!validCoords(x,y)) throw std::runtime_error("Coordinates out of bounds [" + std::to_string(x) + ", " + std::to_string(y) + "]");
    access_count(x,y)++;
    return altitude(x,y);
}
