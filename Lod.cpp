...
Lod::Lod(TerrainMap& m, std::string name_in, Point start_in, Point finish_in)
    : Path(m, name_in, start_in, finish_in) {}

bool Lod::isPointValid(int x, int y, double dz) const {
    if (finish.x == x && finish.y == y) return true;
	double alt = map.alt(x, y);
	if (alt < 0) return true; else return false;
}

double Lod::costFunc(int dx, int dy, double dz) const {
